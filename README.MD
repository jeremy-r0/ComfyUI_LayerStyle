...
## Info
This is a fork of [ComfyUI_LayerStyle](https://github.com/chflame163/ComfyUI_LayerStyle) by chflame163. All credits go to him.
This fork allow the custom nodes to fetch a different path for rmbg 1.4, regardless of casing.
...
* Or open the cmd window in the plugin directory of ComfyUI, like ```ComfyUI\custom_nodes```，type    
  ...
  ```
  git clone https://github.com/jeremy-r0/ComfyUI_LayerStyle.git
  ```
...
* Commit [PadImageBatch](#PadImageBatch) and [RestorePadImageBatch](#RestorePadImageBatch) nodes, used to expand the batch size of video images.
...
### <a id="table1">PadImageBatch</a>  
Extend the Image Batch to the number of frames that conform to the specified rule, such as ```8n+1```.

Node Options:
![image](image/pad_image_batch_node.jpg)
* multiplier: The multiplier for the frame number rule. For example, the "8" in "8n+1"
* remainder: The remainder of the frame number rule. For example, the "1" in "8n+1"
Outputs:
* original_batch_length: Original Frames.

### <a id="table1">RestorePadImageBatch</a>  
Restore the Image Batch to its original quantity after expanding the frame count.

Node Options:
![image](image/restore_pad_image_batch_node.jpg)
* original_batch_length: Original Frames.
...
*Download model files from [BRIA Background Removal v1.4](https://huggingface.co/briaai/RMBG-1.4) or [BaiduNetdisk](https://pan.baidu.com/s/16PMfjpkXn_35T-cVYEPTZA?pwd=qi6o) to ```ComfyUI/models/rmbg/RMBG-1.4``` folder. The folder name is case-insensitive, so ```RMBG```, ```rmbg``` or ```RmBg``` all work. This model can be used for non-commercial purposes.    
